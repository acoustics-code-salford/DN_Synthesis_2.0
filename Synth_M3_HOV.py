...
# cases_list = [[0.3, 3.5, 5, 'aperiodic']] # Best case from previous analysis
...
for cl in range (len(cases_list)):
    ...
    Drone_Recs = [#1500 ["wav file", [low_range,high_rnge], n_har, n_rotors, ee, number ob blades] 
                ["Ed_M3_10_H00_Y_C_nw_ev2_M5.wav", [76, 103], 10, 4, 0.05, 2]
                ]
    n_f = 0
    ...
    if synth_FO == True:
        ope_synth = "flyover"
        velo_uas = "15" # m/s
        Freqs_and_Amplitudes[1][0] = [84.9, 93.5, 95.9, 106.8] #[91, 89, 98, 94]"""hovering BPFs"""
    ...
    # %%% ---- BB AMPLITUDE MODULATION
        ...
        AM_modulator = (1.5*np.sin(2 * np.pi * fm_AM_1*t) + 1.5*np.sin(2 * np.pi * fm_AM_2 * t)+
                        0.5*np.sin(2 * np.pi * fm_AM_1*2*t) + 0.5*np.sin(2 * np.pi * fm_AM_2*2 * t)+
                        0.5*np.sin(2 * np.pi * fm_AM_1*3*t) + 0.5*np.sin(2 * np.pi * fm_AM_2*3 * t)+
                        0.2*np.sin(2 * np.pi * fm_AM_1*4*t) + 0.2*np.sin(2 * np.pi * fm_AM_2*4 * t)
