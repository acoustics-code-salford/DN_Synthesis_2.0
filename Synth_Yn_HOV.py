...
for cl in range (len(cases_list)):
    ...
    Drone_Recs = [#1500 ["wav file", [low_range,high_rnge], n_har, n_rotors, ee, number ob blades] 
                ["Ed_Yn_10_HH_Y_C_nw_ev1_M5.wav", [129,180], 10, 6, 0.05, 2]
                ]
    n_f = 0
    ...
    Q_step = 3 #3 #steps in Q factor for higher frequency notch
    ...
    if BB_method == 'white':
        ...
        fc_modulation_limit = 1000 #10 * np.max([BPFs]) #HF broad band noise 10 times highest BPFs
    ...
    # %%% ---- BB AMPLITUDE MODULATION
        ...
        fm_AM_2 = sum(BPFs[2:4])/2#max(BPFs)#BPFs[1] # frecuency of amplitud modulation BPF 2
        fm_AM_3 = sum(BPFs[4::])/2#max(BPFs)#BPFs[1] # frecuency of amplitud modulation BPF 3
        ...
        AM_modulator = (1.5*(np.sin(2 * np.pi * fm_AM_1*t) + np.sin(2 * np.pi * fm_AM_2 * t)+np.sin(2 * np.pi * fm_AM_3 * t))+
                        0.4*(np.sin(2 * np.pi * fm_AM_1*2*t) + np.sin(2 * np.pi * fm_AM_2*2 * t)+np.sin(2 * np.pi * fm_AM_3*2 * t))
                        #0.1*(np.sin(2 * np.pi * fm_AM_1*2*t) + np.sin(2 * np.pi * fm_AM_2*2 * t)+np.sin(2 * np.pi * fm_AM_3*2 * t))
    ...
    for i_har in range(len(Freqs_and_Amplitudes)):
        ...
        for n_bpf, bpf in enumerate(BPFs_to_add):
            ...
            fm = del_f/max_beta #3*( del_f/max_beta)# Hz 
            
            FM_FACTOR = del_f#del_f*2 # maximum beta factor
            # print(FM_FACTOR)
            ...
            T_h = Abpf/3 * np.sin(2*np.pi * F_h * t + ph_r) # Pure Tone
            T_h_mod =  0.8 * Abpf * np.cos(2*np.pi * F_h * t + FMbeta * np.sin(2*np.pi* fm *t) + PM  + ph_r) #Tone modulated in frequency and rotor-phase
