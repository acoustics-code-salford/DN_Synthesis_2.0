...
for cl in range (len(cases_list)):
    ...
    Drone_Recs = [#1500 ["wav file", [low_range,high_rnge], n_har, n_rotors, ee, number ob blades] 
                ["AS_X1_10_HH_00_18_mic2.wav", [55, 68], 10, 4, 0.05,2 ]
                ]
    n_f = 0
    ...
    position = (0, 0, 5) # x,y,z position of the drone to be backpropagated to.
    ...
    Q_ini = 20 #20 # sarting Q factor 
    Q_step = 5 #3 #steps in Q factor for higher frequency notch
    ...
    # %%% ---- BB AMPLITUDE MODULATION
        ...
        AM_modulator = (1*np.sin(2 * np.pi * fm_AM_1*t) + 1*np.sin(2 * np.pi * fm_AM_2 * t)+
                        0.8*np.sin(2 * np.pi * fm_AM_1*2*t) + 0.8*np.sin(2 * np.pi * fm_AM_2*2 * t)#+
                        #0.5*np.sin(2 * np.pi * fm_AM_1*3*t) + 0.5*np.sin(2 * np.pi * fm_AM_2*3 * t)
                        #0.05*np.sin(2 * np.pi * fm_AM_1*5*t) + 0.05*np.sin(2 * np.pi * fm_AM_2*5 * t)
    ...
    # Q_f = 5 #1.5#15 works fine for both drones
    ...
    for i_har in range(len(Freqs_and_Amplitudes)):
        ...
        for n_bpf, bpf in enumerate(BPFs_to_add):
            ...
            T_h_mod =  0.3* Abpf * np.cos(2*np.pi * F_h * t + FMbeta * np.sin(2*np.pi* fm *t) + PM  + ph_r) #Tone modulated in frequency and rotor-phase
